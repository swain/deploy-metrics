...
weeks so the series is longitudinal, so you can watch throughput trend week
over week rather than seeing a single snapshot.
...
template, never `index.html` directly, or your edits will be overwritten on
the next render.
...
refetched. The in-progress (current) week is not cached. It is refetched every run, since it's still changing.
...
from a classifier change, and it's why the classifier can be changed safely at all. Nothing about the cache design lets stale and fresh classifications
mix silently.
...
to check, because the workflow will not show as failing when it never fires.
